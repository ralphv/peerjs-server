...
app._sendPeers = function(key) {
  if(!this._options.allow_discovery) {
    return;
  }
  if(this._clients[key]) {
    const self = this;
    const destinations = Object.keys(this._clients[key]);
    destinations.forEach(function(id) {
      if(self._clients[key][id] && self._clients[key][id].socket) {
        try {
          self._clients[key][id].socket.send(
            JSON.stringify({type: 'PEERS', payload: destinations}));
        } catch(e) {
